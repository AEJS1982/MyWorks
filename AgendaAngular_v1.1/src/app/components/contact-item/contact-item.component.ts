...
import { currentState } from 'src/app/store/state';
...
export class ContactItemComponent implements OnInit {
  ...
  constructor(private cd:ChangeDetectorRef, private cs:ContactService,private router:Router) { 
    this.model=currentState.selectedPerson;
    this.errors=[];
    this.editMode=false;
  }

  ngOnInit(): void {
    if (currentState.selectedPerson.id>0)
      this.editMode=true;

    this.model=currentState.selectedPerson;
    console.log(currentState.selectedPerson.id);
  }
